...
class DataSourceBase(ABC):
    ...
    def normalize_company_name(self, company: Optional[str]) -> Optional[str]:
        ...
        mappings = {
            ...
            "cruise": ["cruise", "cruise llc", "gm cruise", "cruise automation", "general motors cruise"],
    ...
    def normalize_incident_type(self, incident_type: Optional[str]) -> str:
        ...
        # Collision types
        if any(term in type_lower for term in ["collision", "crash", "accident", "impact", "struck"]):
            return "collision"
        
        # Near miss types
        if any(term in type_lower for term in ["near miss", "near-miss", "close call", "avoided"]):
            return "near_miss"
        
        # Sudden behavior types  
        if any(term in type_lower for term in ["sudden", "abrupt", "unexpected", "erratic", "swerve"]):
            return "sudden_behavior"
        
        # Blockage types
        if any(term in type_lower for term in ["block", "obstruction", "stopped", "stuck", "immobile"]):
            return "blockage"
