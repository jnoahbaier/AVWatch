...
class CaliforniaDMVDataSource(DataSourceBase):
    ...
    async def fetch_data(self, since: Optional[datetime] = None) -> list[dict[str, Any]]:
        ...
        if since:
            reports = [
                r for r in reports
                if r.get("date") and r["date"] >= since
            ]
    ...
    def _parse_date(self, text: str) -> Optional[datetime]:
        ...
        # Common formats in DMV reports
        formats = [
            r"(\w+)\s+(\d{1,2}),?\s+(\d{4})",  # December 4, 2025
            r"(\d{1,2})/(\d{1,2})/(\d{4})",     # 12/04/2025
            r"(\w+)\s+(\d{4})",                  # December 2025 (fallback)
        ]
...
class CaliforniaDMVDisengagementSource(DataSourceBase):
    ...
    BASE_URL = "https://www.dmv.ca.gov"
    REPORTS_URL = "/portal/vehicle-industry-services/autonomous-vehicles/disengagement-reports/"
