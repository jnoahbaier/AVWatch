...
class NHTSAComplaintsAPI(DataSourceBase):
    ...
    def __init__(self, makes: Optional[list[str]] = None, years: Optional[range] = None):
        """
    ...
    async def fetch_data(self, since: Optional[datetime] = None) -> list[dict[str, Any]]:
            ...
            for make in self.makes:
                for year in self.years:
                    try:
                        ...
                        self.logger.warning(f"Failed to fetch complaints for {year} {make}: {e}")
        ...
        if since:
            av_complaints = [
                c for c in av_complaints
                if self._parse_complaint_date(c) and self._parse_complaint_date(c) >= since
            ]
    ...
    def _filter_av_related(self, complaints: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """Filter complaints to only include AV/ADAS related issues."""
        av_keywords = [
            "autopilot", "autosteer", "auto pilot", "auto steer",
            "full self driving", "fsd", "self-driving", "self driving",
            "autonomous", "automated", "driver assist", "adas",
            "cruise control", "lane keeping", "lane departure",
            "collision avoidance", "automatic emergency", "aeb",
            "phantom braking", "phantom brake",
            "tesla vision", "waymo", "cruise", "zoox",
        ]
        ...
        for complaint in complaints:
            ...
            if any(keyword in summary or keyword in components for keyword in av_keywords):
    ...
    def _parse_complaint_date(self, complaint: dict[str, Any]) -> Optional[datetime]:
        ...
        date_str = complaint.get("dateOfIncident") or complaint.get("dateComplaintFiled")
        if not date_str:
            return None
...
class NHTSARecallsAPI(DataSourceBase):
            ...
            for make in self.makes:
                for year in self.years:
                    try:
                        ...
                        self.logger.warning(f"Failed to fetch recalls for {year} {make}: {e}")
    ...
    def _filter_av_related(self, recalls: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        av_keywords = [
            "autopilot", "autosteer", "self-driving", "autonomous",
            "driver assist", "adas", "cruise control", "lane keeping",
            "collision avoidance", "automatic emergency braking",
            "forward collision", "camera", "sensor", "radar", "lidar",
        ]
        ...
        for recall in recalls:
            ...
            if any(keyword in summary or keyword in component for keyword in av_keywords):
...
class NHTSAFarsAPI(DataSourceBase):
    ...
    def __init__(self, states: Optional[list[int]] = None, years: Optional[range] = None):
        """
        Initialize FARS API.
            ...
            for state in self.states:
                for year in self.years:
                    try:
                        ...
                        self.logger.warning(f"Failed to fetch FARS for state {state}, year {year}: {e}")
    ...
    def _parse_crash(self, crash: dict[str, Any]) -> Optional[IncidentRecord]:
        ...
        return IncidentRecord(
            ...
            injuries=int(crash.get("PERSONS", 0) or 0) - int(crash.get("FATALS", 0) or 0),
            confidence_score=1.0,
            status="verified",
            raw_data=crash,
        )
