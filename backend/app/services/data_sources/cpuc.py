...
class CPUCDataSource(DataSourceBase):
    ...
    async def fetch_data(self, since: Optional[datetime] = None) -> list[dict[str, Any]]:
        ...
        if since:
            reports = [
                r for r in reports
                if r.get("period_end") and r["period_end"] >= since
            ]
    ...
    def _parse_report_link(self, text: str, href: str) -> Optional[dict[str, Any]]:
        ...
        company = None
        known_companies = [
            "Waymo", "Cruise", "Zoox", "Aurora", "AutoX", "WeRide",
            "Tensor", "Motional", "Ghost", "Pony", "Argo"
        ]
        ...
        if match:
            ...
            period_end = datetime(year, quarter * 3, 
        ...
        if match:
            ...
            period_end = datetime(year, end_month, 
        ...
        if match and not period_start:
            ...
            if month:
                ...
                period_end = datetime(year, month,
        ...
        try:
            with zipfile.ZipFile(io.BytesIO(zip_content)) as zf:
                for filename in zf.namelist():
                    ...
                    if any(term in filename.lower() for term in ["incident", "collision", "complaint"]):
