...
class NHTSASGODataSource(DataSourceBase):
    ...
    async def fetch_data(self, since: Optional[datetime] = None) -> list[dict[str, Any]]:
        ...
        async with httpx.AsyncClient(timeout=self.config.timeout_seconds) as client:
            ...
            if self.include_other:
                other_records = await self._fetch_csv(client, self.OTHER_CSV_URL, "OTHER")
    ...
    def _parse_single_record(self, row: dict[str, Any]) -> Optional[IncidentRecord]:
        ...
        if not occurred_at:
            ...
            occurred_at = self._parse_submission_date(row.get("Report Submission Date", ""))
        ...
        record = IncidentRecord(
            ...
            status="verified",
            raw_data=row,
            reported_at=self._parse_submission_date(row.get("Report Submission Date", "")),
        )
    ...
    def _determine_incident_type(self, row: dict[str, Any]) -> str:
        ...
        if any(word in narrative for word in ["struck", "hit", "collision", "crash", "impact"]):
            return "collision"
    ...
    def _extract_injury_info(self, row: dict[str, Any]) -> tuple[int, int]:
        ...
        severity = row.get("Injury Severity", row.get("Highest Injury Severity", "")).lower()
